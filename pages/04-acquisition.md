...
# Survey plan
Acquisition of the MBES data should follow the pre-survey plan discussed in [section 2](#heading=h.44sinio), unless the on-board seabed mapping lead decides otherwise based on the environmental situation and new information at-hand, which are difficult to account for in the planning stage. It is recommended that any changes to the acquisition plan are captured in the Report of Survey ([section 6.2.2](#heading=h.1d96cc0)). Wherever possible, nearing the conclusion of data acquisition, a review of data coverage is highly recommended and infill lines conducted to ensure there are no gaps in the bathymetry, as this impacts the suitability of the data for end use. Additional lines over significant shoal features are also recommended to ensure good density of soundings and determination of least depth. For efficiency, such lines may be conducted concurrently to other activities such as during transits or seabed sampling. Emphasis here is put on the system settings and other specifics that were not recommended in [section 2](#heading=h.44sinio), especially [section 2.5](#heading=h.4f1mdlm).
...
*   project structure:
    ...
    3. QA DataPack
    4. products
    5. raw data ([see 2.3.1](#heading=h.qsh70q))
    6. processed data
    7. backscatter
    8. WC data
...
# _Transit data_
It is recommended that the system settings during transit data acquisition be set to maximise data quality by considering the overall characteristics of the transit rather than maximise data coverage or swath width. Refer also to [section 2.5.5](#heading=h.111kx3o).
...
During the survey, data QC should be done using predicted tides from the [Bureau of Meteorology](http://www.bom.gov.au/australia/tides/) (BOM) for standard ports or [AusTides](http://www.hydro.gov.au/prodserv/publications/ausTides/tides.htm) for secondary ports. Refer also to [section 2.5.2](#heading=h.28h4qwu).

# Monitoring, QA/QC & Data backup
During a survey the following information should be constantly monitored, including:
*   depth
*   vessel draft
*   GNSS (see [section 4.5.1](#heading=h.319y80a)) or subsea positioning for sub-sea platform
*   motion sensor
*   sound velocity
...
To ensure safe data transport it is recommended that multiple copies of the data be made and transported separately in the time between data collection and submission ([section 7](#heading=h.3x8tuzt)).

# _GNSS positioning_
Most seafloor mapping and GNSS software provide real-time monitoring capabilities. The quality of the GNSS data should be monitored while mapping to ensure that the horizontal positioning falls within the seafloor mapping specification. Any deviations outside of the survey specification should be noted and included within the Report of Survey ([section 6.2](#heading=h.1d96cc0)). Maintaining a minimum QC requirement will provide data that is interoperable with many providers and uses. This integrity information includes (LINZ, 2016):
...
# _Dangers found – hydrographic notes_
...
# _Underwater cultural heritage notification_
